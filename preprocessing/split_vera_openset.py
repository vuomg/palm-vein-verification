"""
split_vera_openset.py

Split VERA enhanced dataset into train/test for open-set verification.
Follows the same protocol as final_dataset_openset (70/30 split at identity level).

Identity = subject × hand (e.g. "001_L", "001_R") → 220 identities total.

Usage:
    python split_vera_openset.py
    python split_vera_openset.py --input VERA_enhanced --output VERA_dataset_openset --seed 42
"""
...
def split_vera_openset(
    input_dir: str = "VERA_enhanced",
    output_dir: str = "VERA_dataset_openset",
    train_ratio: float = 0.7,
    seed: int = 42,
):
    """
    Split VERA enhanced dataset into train/test (open-set protocol).

    Identity-level split — train and test identities are completely disjoint.
    70% identities → train, 30% identities → test.

    Args:
        input_dir: Path to VERA_enhanced directory
    ...
    if not input_path.exists():
        ...
        print(f"Run prepare_vera_dataset.py first.")
        return

    # Each subdirectory is an identity class (e.g. "001_L")
    ...
    split_info = {
        ...
        "task_type": "open-set verification (VERA dataset)",
    ...
    print("\n" + "=" * 60)
    print("VERA Open-Set Split Summary")
    print("=" * 60)
    ...
    print("=" * 60)
    print("\nNext step:")
    print(f'   run_vera_baselines.bat')

    return split_info


def main():
    parser = argparse.ArgumentParser(description="Split VERA dataset for open-set verification")
    parser.add_argument("--input", type=str, default="VERA_enhanced",
                        help="Input directory (output of prepare_vera_dataset.py)")
    parser.add_argument("--output", type=str, default="VERA_dataset_openset",
                        help="Output directory (default: VERA_dataset_openset)")
