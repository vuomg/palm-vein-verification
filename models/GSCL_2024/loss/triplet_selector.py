...
class FunctionNegativeTripletSelector(TripletSelector):
    ...
    def get_triplets(self, embeddings, labels):
        ...
        # Handle case when no valid triplets were found
        if len(triplets) == 0:
            # Create a fallback triplet from available indices
            # Find any class with at least 2 samples
            unique_labels = list(set(labels))
            fallback_triplet_created = False
            
            for label in unique_labels:
                label_indices = np.where(labels == label)[0]
                negative_indices = np.where(labels != label)[0]
                
                if len(label_indices) >= 2 and len(negative_indices) >= 1:
                    # Use first two samples of this class as anchor-positive
                    triplets.append([label_indices[0], label_indices[1], negative_indices[0]])
                    fallback_triplet_created = True
                    break
            
            # If still no triplet (edge case: all samples same class or single sample per class)
            if not fallback_triplet_created:
                # Return dummy triplet with first 3 indices (will produce zero loss)
                n_samples = len(labels)
                if n_samples >= 3:
                    triplets.append([0, 1, 2])
                else:
                    triplets.append([0, 0, 0])  # Degenerate case
